...
use script_traits::{AnimationState, AnimationTickType};
...
use webrender;
use webrender_api::{self, ClipId, DeviceUintRect, DeviceUintSize, HitTestFlags, HitTestResult};
use webrender_api::{LayoutPoint, LayoutVector2D, ScrollClamping, ScrollEventPhase, ScrollLocation};
...
impl<Window: WindowMethods> IOCompositor<Window> {
    ...
    fn dispatch_mouse_window_event_class(&mut self, mouse_window_event: MouseWindowEvent) {
        ...
        let node_address = Some(result.tag.0);
    ...
    fn dispatch_mouse_window_move_event_class(&mut self, cursor: TypedPoint2D<f32, DevicePixel>) {
        ...
        if let Some(item) = results.items.first() {
            let event = MouseMoveEvent(Some(item.point_in_viewport.to_untyped()), Some(item.tag.0));
            ...
            let cursor = Cursor::from_u8(item.tag.1).ok().unwrap();
    ...
    fn send_touch_event(&self,
                        event_type: TouchEventType,
                        identifier: TouchId,
                        point: TypedPoint2D<f32, DevicePixel>) {
        let results = self.hit_test_at_point(point);
        if let Some(item) = results.items.first() {
            let event = TouchEvent(event_type,
                                   identifier,
    ...
    pub fn on_touchpad_pressure_event(&self,
        ...
        let results = self.hit_test_at_point(point);
        if let Some(item) = results.items.first() {
            let event = TouchpadPressureEvent(item.point_in_viewport.to_untyped(),
                                              pressure,
                                              phase,
                                              Some(item.tag.0));
            let pipeline_id = PipelineId::from_webrender(item.pipeline);
