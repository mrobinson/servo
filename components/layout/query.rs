...
pub fn process_resolved_style_request(
    ...
    let get_fragment_details = |fragment: &Fragment| -> (
        Rect<Au, CSSPixel>,
        ...
        if let Some(box_fragment) = fragment.retrieve_box_fragment() {
            ...
            return (content_rect, margins, padding, specific_layout_info);
        }
        (
            fragment.base().map(|base| base.rect()).unwrap_or_default(),
            SideOffsets2D::zero(),
            SideOffsets2D::zero(),
            None,
        )
    ...
    let resolve_for_fragment = |fragment: &Fragment| {
        ...
        let (content_rect, margins, padding, specific_layout_info) = get_fragment_details(fragment);
