...
impl SpecificInputType for RadioInputType {
    ...
    fn bind_to_tree(&self, input: &HTMLInputElement, cx: &mut JSContext, _context: &BindContext) {
        radio_group_updated(input, input.radio_group_name().as_ref(), CanGc::from_cx(cx));
    }

    fn unbind_from_tree(&self, input: &HTMLInputElement, context: &UnbindContext, can_gc: CanGc) {
        let form_owner = input.form_owner();
