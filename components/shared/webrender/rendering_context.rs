...
pub trait RenderingContext {
    ...
    /// the OpenGL texture object. The size of the texture is the size of the
    /// bound surface. Default to `None`.
    fn create_texture(&self, _surface: Surface) -> Option<(SurfaceTexture, u32)> {
        None
    }
...
impl SurfmanRenderingContext {
    ...
    fn create_texture(&self, surface: Surface) -> Option<(SurfaceTexture, u32)> {
        let device = &self.device.borrow();
        let context = &mut self.context.borrow_mut();
        let SurfaceInfo {
            id: front_buffer_id,
