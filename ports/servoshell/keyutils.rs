...
use winit::keyboard::{
    Key as WinitKey, KeyCode as WinitKeyCode, KeyLocation as WinitLocation, ModifiersState,
    NamedKey as WinitNamedKey, PhysicalKey as WinitPhysicalKey,
};
...
fn get_servo_key_from_key_event(event: &KeyEvent) -> Key {
    match event.logical_key {
        WinitKey::Named(named_key) => match named_key {
            ...
            // Space is considered printable ie `Unidentified`.
            WinitNamedKey::Space => Key::Unidentified,
        ...
        WinitKey::Character(ref string) => Key::Character(string.to_string()),
        WinitKey::Unidentified(_) => Key::Unidentified,
        WinitKey::Dead(_) => Key::Unidentified,
    }
}

fn get_servo_location_from_key_event(event: &KeyEvent) -> Location {
    match event.location {
        WinitLocation::Standard => Location::Standard,
        WinitLocation::Left => Location::Left,
        WinitLocation::Right => Location::Right,
        WinitLocation::Numpad => Location::Numpad,
    }
}

fn get_servo_keycode_from_key_event(event: &KeyEvent) -> Code {
    match event.physical_key {
        WinitPhysicalKey::Code(code) => match code {
            ...
            WinitKeyCode::Meta => Code::MetaLeft,
...
fn get_modifiers(state: ModifiersState) -> Modifiers {
...
pub fn keyboard_event_from_winit(event: &KeyEvent, state: ModifiersState) -> KeyboardEvent {
    info!("Received winit keyboard input: {:?}", event);
    KeyboardEvent {
        state: match event.state {
            ElementState::Pressed => KeyState::Down,
            ElementState::Released => KeyState::Up,
        },
        key: get_servo_key_from_key_event(event),
        code: get_servo_keycode_from_key_event(event),
        location: get_servo_location_from_key_event(event),
